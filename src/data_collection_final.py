...
while True:
    try:
        ...
        if hands:
            hand = hands[0]
            bbox_data = hand.get('bbox')  # Get the value associated with 'bbox'

            if isinstance(bbox_data, list):  # Check if bbox_data is a list
                x, y, w, h = bbox_data  # Unpack values if it's a list
            else:
                print("Error: 'bbox' key is not a list. Skipping hand processing.")
